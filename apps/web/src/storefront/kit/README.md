...
> layout yet, and `kit.css` is intentionally **not imported anywhere**, so
> the build stays non-breaking. Adopt them deliberately (see below).

## Design tokens

Defined in `kit.css` under `:root`:
...
`kit.css` `@import`s Space Grotesk + Inter from Google Fonts, so importing
...
1. Import the stylesheet **once** in the storefront layout:
   ...
   ```
2. Wrap the storefront tree in `<StorefrontRoot>` (renders `className="storefront"`
   and injects the tenant's BrandConfig colors as CSS variables) so the base
   reset + token defaults apply (the reset is scoped to `.storefront` so it does
   not affect other pages):
   ```tsx
   <StorefrontRoot>{/* ...page... */}</StorefrontRoot>
   ```
...
| `StorefrontNav`    | Lowercase brand + lime dot, links, dark cart pill    | `brand?`, `links?`, `cartCount?`, `cartHref?` |
...
| `StorefrontFooter` | Dark footer, lime headings, columns, bottom bar      | `brand?`, `blurb?`, `columns?`, `legal?`, `socials?` |
